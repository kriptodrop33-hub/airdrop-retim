...
async def _do_research(update: Update, context: ContextTypes.DEFAULT_TYPE, input_text: str):
    ...
    try:
        ...
        # Custom emoji veya diğer HTML hatası -> Sadece tg-emoji taglarını sil ve tekrar dene
        ...
        try:
            ...
            logger.error(f"Post önizleme HTML hatası (2): {e2}")
            # Hala hata varsa tüm HTML'i sil
            plain = re.sub(r'<[^>]+>', '', no_emoji_tags)
            await update.effective_message.reply_text(
                f"✅ Post hazır ama HTML desteklenmiyor ({e2}).\n\n{plain}",
                reply_markup=post_actions_extended(has_link=False, fmt="long", score=score),
            )
...
async def _send_to_group(update: Update, context: ContextTypes.DEFAULT_TYPE, with_photo: bool):
    ...
    try:
        ...
        target = update.callback_query.message if update.callback_query else update.message
        # HTML parse hatası durumunda önce tg-emoji'leri silip tekrar dene
        no_emoji = re.sub(r'</?tg-emoji[^>]*>', '', send_text)
        try:
            if with_photo and img_url:
                await context.bot.send_photo(
                    chat_id=GROUP_CHAT_ID,
                    photo=img_url,
                    caption=no_emoji[:1024] if len(no_emoji) > 1024 else no_emoji,
                    parse_mode=ParseMode.HTML,
                )
            else:
                await context.bot.send_message(
            ...
            await target2.reply_text("⚠️ Premium emoji hatası — Standart emojilerle gruba gönderildi.", reply_markup=main_menu())
        except Exception as e2:
            logger.error(f"Gönderme düz metin hatası: {e2}")
            try:
                await context.bot.send_message(
                    chat_id=GROUP_CHAT_ID,
                    text=re.sub(r'<[^>]+>', '', no_emoji),
                )
                target2 = update.callback_query.message if update.callback_query else update.message
                await target2.reply_text("⚠️ HTML hatası — düz metin olarak gruba gönderildi.", reply_markup=main_menu())
            except Exception as e3:
                await target.reply_text(f"❌ Gönderim hatası: <code>{e3}</code>", parse_mode=ParseMode.HTML)
...
async def handle_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if data == "home":
        ...
        try:
            ...
            logger.error(f"regen_post HTML hatası: {e}")
            no_emoji = re.sub(r'</?tg-emoji[^>]*>', '', preview)
            try:
                await msg.edit_text(no_emoji, parse_mode=ParseMode.HTML, reply_markup=post_actions(has_link=False, fmt=fmt))
            except Exception:
        ...
        try:
            ...
            logger.error(f"fmt callback HTML hatası: {e}")
            no_emoji = re.sub(r'</?tg-emoji[^>]*>', '', preview)
            try:
                await msg.edit_text(no_emoji, parse_mode=ParseMode.HTML, reply_markup=post_actions(has_link=False, fmt=fmt))
            except Exception:
